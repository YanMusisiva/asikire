    ...
    <main className="bg-[#050505] min-h-screen">
      ...
      <div className="max-w-7xl mx-auto px-6 pt-36 pb-28">
        ...
        <div className="mb-16">
          ...
          <p className="text-white/40 max-w-xl leading-relaxed">{t("blog.sub")}</p>
        </div>
            ...
            {articles.map((article) => (
              ...
              >
                <div className="flex items-center justify-between mb-5">
                  ...
                  {article.readTime && (
                    <span className="font-mono text-[10px] text-white/20">{article.readTime}</span>
                  )}
                </div>
                <h2 className="font-display text-lg font-bold text-white mb-3 group-hover:text-[#22c55e] transition-colors leading-snug">
                  {article.title}
                </h2>
                <p className="text-white/40 text-sm leading-relaxed flex-1 mb-5">{article.excerpt}</p>
